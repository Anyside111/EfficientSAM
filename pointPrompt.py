...
def process_folders(training_file_path, label_file_path, output_folder, predict_output_folder):
    ...
    if not os.path.exists(predict_output_folder):
        os.makedirs(predict_output_folder)
    ...
    for slice_index in range(label_data.shape[2]):
        ...
        if np.any(label_slice > 0):
            ...
            predict_mask = np.stack([predict_mask] * 3, axis=-1).astype(
                np.uint8) * 255  # Convert single channel to 3 channels
            ...
            cv2.imwrite(os.path.join(predict_output_folder, f'{img_index}_{slice_index}.png'), predict_mask)
...
for filename in os.listdir(training_data_folder):
    if filename.endswith('.nii.gz') and not filename.startswith('._'):
        ...
        process_folders(training_file_path, label_file_path, output_folder, predict_output_folder)
    else:
